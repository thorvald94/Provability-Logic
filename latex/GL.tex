...
This chapter (and the following one) does not use the preceding sections about recursive functions. We will only
...
. Due to this theorem get that we can without loss of generality assume that
...
\begin{proof}
	\textbf{Ændre alle T til mathcal{T}}
	The implications $(1)\Rightarrow (2)$ and $(1)\Rightarrow(3)$ follow
	by the completeness theorem \textbf{Kryds ref}. We also have that $(2)\Leftrightarrow (3)$
...
which we assume to be  a counter model i.e
	$\not\vDash_{w_0}^{K}\alpha$. We can know that such a counter-model
	...
	The model $\mathcal{K}_T$ then further consists of:
	\begin{enumerate}
		\item $<_T$ is the usual strict
	ordering by extension of finite sequences. 
		\item	The world $w_0$ which is the root node.
		\item$\phi_T$ is the defined in
		the following way:
	...
	we go from $(w_0,\ldots w_n)$ to $(w_0,\ldots, w_n,w_{n+1})$ we are one
	...
	\end{align*}
	And the last line is the same as $\vDash^{\mathcal{T}}_{(w_0,\ldots,w_n)}\Box\gamma$.
	...
	$\not\vDash^\mathcal{K}_{(w_0,\ldots,w_n)}\Box\gamma$. 
...
any deeper explenation for the name.
...
\parencite{Lind1997} since it is a lot easier to understand and work with.
...
\begin{thm}
	...
	subformulae of $\alpha$ that is boxed. Then the following to
