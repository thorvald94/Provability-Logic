...
The most definitions and results from this section will be used in the rest of
the project.
...
\parencite{Lemmon1977} and \parencite{Blackburn2002}. Here we will just state a few of
...
primitive recursive arithmetic and the notion of Gödel numbering.
...
\begin{defi}
	...
	\begin{enumerate}
		...
		\item From $G\rightarrow F v$ derive $G\rightarrow
			...
			$v$ occurs free in $G$.
	\end{enumerate}
	A \textit{formal derivation} in \PRA\ is a sequence of formulas of \PRA\
...
The predicate $\text{Pr}(x)$ can been shown to having the following properties called Löb's
...
\end{enumerate}
It is also clear that $\neg\Pr(\Godelnum{\bot})$, where $\bot$ is some false statement,
expresses consistence of \PRA. We will denote this by $\text{Con}$.
...
$\Box$-operator of \GL, we get the following table:
	...
	about the logic basis of ethics \parencite{Smil1963}. Further the modal
