...
\begin{defi}
	...
	We will from now on  denote the graph of a function $\varphi(\x)=y$
	by\\ $\tau\x
	y$
\end{defi}
...
the following lemma:
\begin{lem}
	Let $\tau v_0\ldots v_{n-1}$ be a $\Sigma_1$. Then there is a
	recursively enumerable formula such that 
	\[\PRA\vdash\tau v_0\ldots v_{-n1}\leftrightarrow\exists v(fvv_0\ldots
	v_{n-1}=\ol 0)\]
\end{lem}
\begin{proof}
\end{proof}
